...
CONFIG_NAME = "default_run_token_obs_mlp_pdm_score"


def _compute_trajectory_with_optional_token(
    agent: AbstractAgent,
    agent_input: AgentInput,
    token: str,
    scene: Scene | None = None,
):
    """Call compute_trajectory while passing token when the agent supports it."""
...
def run_token_obs_mlp_pdm_score(args: List[Dict[str, Union[List[str], DictConfig]]]) -> List[pd.DataFrame]:
    """
    Worker function for PDM scoring with token-named npy observations.
    :param args: input worker data points grouped by log.
    :return: per-token score rows.
    """
    node_id = int(os.environ.get("NODE_RANK", 0))
    thread_id = str(uuid.uuid4())
    logger.info(f"Starting token-obs MLP worker in thread_id={thread_id}, node_id={node_id}")
    ...
    for idx, token in enumerate(tokens_to_evaluate_stage_one):
        logger.info(
            f"Processing stage one reactive scenario {idx + 1} / {len(tokens_to_evaluate_stage_one)} "
        ...
        try:
            ...
            trajectory = _compute_trajectory_with_optional_token(agent, agent_input, token, scene)
            ...
            logger.warning(f"----------- Token-obs MLP agent failed for token {token}:")
            traceback.print_exc()
    ...
    for idx, token in enumerate(tokens_to_evaluate_stage_two):
        logger.info(
            f"Processing stage two reactive scenario {idx + 1} / {len(tokens_to_evaluate_stage_two)} "
            f"in thread_id={thread_id}, node_id={node_id}"
        )
        try:
            metric_cache = metric_cache_loader.get_from_token(token)
            ...
            score_row_stage_two["ego_simulated_states"] = [ego_simulated_states]
...
def main(cfg: DictConfig) -> None:
    """
    Main entrypoint for running PDM evaluation from token-named GPUDrive obs npy files.
    ...
    logger.info(f"Starting token-obs MLP PDM scoring of {len(tokens_to_evaluate)} scenarios...")
    data_points = [
        {
            "cfg": cfg,
            "log_file": log_file,
    ...
    score_rows: List[pd.DataFrame] = worker_map(worker, run_token_obs_mlp_pdm_score, data_points)
    ...
    logger.info(
        f"""
        Finished running token-obs MLP evaluation.
