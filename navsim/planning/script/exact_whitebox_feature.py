...
#         SemanticMapLayer.STOP_LINE: 6,       # StopSign（近似）
#         SemanticMapLayer.ROADBLOCK: 2,       # RoadEdge（近似）
#     }
...
#             point=query_point, radius=80.0, layers=layers
#         )
#     except Exception:
#         return np.zeros((ROAD_NUM, ROAD_DIM), dtype=np.float32)
...
#                 try:
#                     seg_width = float(obj.get_width())  # nuPlan lane 有此方法
#                 except Exception:
#                     seg_width = 3.7
#                 seg_height = 0.0
...
#     # Sort by distance, take closest ROAD_NUM
#     rows.sort(key=lambda x: x[0])
#     result = np.zeros((ROAD_NUM, ROAD_DIM), dtype=np.float32)
#     count = min(len(rows), ROAD_NUM)
